...
class RichText implements Converter
{
    ...
    protected function reportNonUniqueIds(DOMDocument $document)
    {
        $xpath = new DOMXPath($document);
        ...
        foreach ($nodes as $node) {
            ...
            if ($this->logger !== null) {
                $this->logger->warning("Duplicated id in original ezxmltext for contentobject_attribute.id=$this->currentContentFieldId, automatically generated new id : $duplicatedId --> $id");
            }
        }
    }
     ...
     * @return bool
     */
    protected function isImageContentType($id, $isContentId = true)
    {
        try {
            if ($isContentId) {
            ...
            $this->logger->warning("Unable to find content_id=$id, referred to in embedded tag in contentobject_attribute.id=$this->currentContentFieldId.");
    ...
    public function tagEmbeddedImages(DOMDocument $richtextDocument)
    {
        ...
        foreach ($nodes as $node) {
            ...
            $isImage = $this->isImageContentType($id, $isContentId);
            if ($isImage) {
